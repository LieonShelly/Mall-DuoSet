...
- (void)tapSubmitBtn:(id)sender {
    ...
    if ([dict objectForKey:@"tel"]) {
        NSString *str = [dict objectForKey:@"tel"];
        NSString *valiStr = [ValiMobile valiMobile:str];
        if (![valiStr isEqualToString:@"YES"]) {//检验是否是手机号码
            [MQToast showToast:valiStr duration:1.0 window:[[UIApplication sharedApplication].windows lastObject]];
            return;
        }
    }
