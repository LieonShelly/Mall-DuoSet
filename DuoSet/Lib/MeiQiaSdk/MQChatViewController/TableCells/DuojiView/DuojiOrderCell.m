...
@interface DuojiOrderCell()

@end

@implementation DuojiOrderCell{
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        productNameLable.font = CUSFONT(13);
...
- (void)updateCellWithCellModel:(id<MQCellModelProtocol>)model {
    if (![model isKindOfClass:[DuojiOrderModel class]]) {
        NSAssert(NO, @"传给 MQBotAnswerCellModel.h 的Model类型不正确");
        return ;
    }
    DuojiOrderModel *cellModel = (DuojiOrderModel *)model;
    //刷新头像
    ...
    productNameLable.text = [NSString stringWithFormat:@"订单编号:%@",cellModel.message.orderNo];
    subLable.text = cellModel.message.productName;
    priceLable.text = [NSString stringWithFormat:@"￥ %@",cellModel.message.price];
}
@end
