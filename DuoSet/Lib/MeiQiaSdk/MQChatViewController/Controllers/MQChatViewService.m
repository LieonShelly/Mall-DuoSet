                         ...
                         completeStatus:(BOOL)completion
{
    ...
    if (receivedMessages) {
        ...
        if (_productCellModel) {
            NSMutableArray *arr = [NSMutableArray arrayWithArray:_cellModels];
            [arr addObject:_productCellModel];
            _cellModels = arr;
            [self.delegate insertCellAtTopForModelCount: newCellCount + 1];
        }else if (_orderModel){
            NSMutableArray *arr = [NSMutableArray arrayWithArray:_cellModels];
...
- (void)updateChatTitleWithAgent:(MQAgent *)agent state:(MQState)state {
    ...
    if (self.delegate) {
        ...
        if ([self.delegate respondsToSelector:@selector(changeNavReightBtnWithAgentType:hidden:)]) {
            NSString *agentType = @"";
#warning 美洽导航栏的右上角按钮，当前项目只要电话客服
...
- (void)handleEventMessage:(MQEventMessage *)eventMessage {
    NSString *tipString = eventMessage.content;
    if (tipString.length > 0) {
        if ([self respondsToSelector:@selector(didReceiveTipsContent:)]) {
            [self didReceiveTipsContent:tipString showLines:true];
        }
    }
...
- (void)didReceiveTipsContent:(NSString *)tipsContent {
    [self didReceiveTipsContent:tipsContent showLines:false];
}
