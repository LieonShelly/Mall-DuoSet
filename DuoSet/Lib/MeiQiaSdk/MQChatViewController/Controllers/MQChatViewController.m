...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(meiQiaProdcutCellChatNotifyHandle) name:@"MeiQiaProdcutCellChatNotify" object:nil];
...
- (void)initchatViewService {
    ...
    if (_productCellModel) {
        self.chatViewService.productCellModel = _productCellModel;
    }
    if (_orderModel) {
        self.chatViewService.orderModel = _orderModel;
    }
}
...
- (MQInputToolBar *)chatInputBar {
    if (!_chatInputBar) {
        ...
        _chatInputBar = [[MQInputToolBar alloc] initWithFrame:CGRectMake(10, 0, self.view.frame.size.width - 10 - 100, kMQChatViewInputBarHeight) contentView: contentView];
...
//美洽通知
-(void)meiQiaProdcutCellChatNotifyHandle{
    [self dismissChatViewController];
}
