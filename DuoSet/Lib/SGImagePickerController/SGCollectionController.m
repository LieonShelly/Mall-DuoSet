...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell.selectedButton == nil) {//防止多次创建
        ...
        [selectButton setImage:[UIImage imageNamed:@"choose_default"] forState:UIControlStateNormal];
        [selectButton setImage:[UIImage imageNamed:@"choose_selected"] forState:UIControlStateSelected];
        selectButton.imageView.contentMode = UIViewContentModeCenter;
        CGFloat width = cell.bounds.size.width;
        selectButton.frame = CGRectMake(width - 40, 0, 40, 40);
        selectButton.imageEdgeInsets = UIEdgeInsetsMake(-20, 0, 0, -20);
...
- (void)buttonClicked:(UIButton *)sender{
    ...
    if (sender.selected == YES) {//选中了记录
        if (self.maxCount <= 0) {
            [SGTip loadTipWithContent:@"最多只能选择9张" InView:self.view];
            sender.selected = NO;
        }else{
