...
-(void)timerAction:(NSTimer *)timer{
    ...
    if((self.offsetX > -firstRect.size.width && self.orientation == RollingOrientationLeft) ||
        ...
        //自定义红色文字
        NSString *textStr = [labelTextArray firstObject];
        NSString *beeStr = [self.beeCountArr firstObject];
        NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc]initWithString:textStr];
        [attributeString addAttribute:NSFontAttributeName value:CUSNEwFONT(12) range:NSMakeRange(textStr.length - 3 - beeStr.length, beeStr.length)];
        [attributeString addAttribute:NSForegroundColorAttributeName value:[UIColor mainColor] range:NSMakeRange(textStr.length - 3 - beeStr.length, beeStr.length)];
        _labels[0].attributedText = attributeString;
...
//        _labels[1].text = [labelTextArray lastObject];
        //自定义红色文字
        NSString *textStr2 = [labelTextArray lastObject];
        NSString *beeStr2 = [self.beeCountArr lastObject];
        NSMutableAttributedString *attributeString2 = [[NSMutableAttributedString alloc]initWithString:textStr2];
        [attributeString2 addAttribute:NSFontAttributeName value:CUSNEwFONT(12) range:NSMakeRange(textStr2.length - 3 - beeStr2.length, beeStr2.length)];
        [attributeString2 addAttribute:NSForegroundColorAttributeName value:[UIColor mainColor] range:NSMakeRange(textStr2.length - 3 - beeStr2.length, beeStr2.length)];
        _labels[1].attributedText = attributeString2;
    }
