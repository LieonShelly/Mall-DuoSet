...
@interface ListSearchView()<UITableViewDelegate,UITableViewDataSource, UITextFieldDelegate>
{
...
-(void)configData{
    [RequestManager requestWithMethod:GET WithUrlPath:@"product/keyword" params:nil from:nil showHud:false loadingText:nil enableUserActions:false success:^(id responseDic) {
...
-(void)configUI{
    ...
    [self addSubview:_correlationView];
}
...
- (void)configCell:(MKJTagViewTableViewCell *)cell indexpath:(NSIndexPath *)indexpath
{
    ...
    cell.tagView.didTapTagAtIndex = ^(NSUInteger index,UIButton *btn,NSArray *tagBtns)
    {
        ...
        ChoiceSearchDataBlock block = _choiceHanld;
        if (block) {
            block(itme);
        }
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    if (textField.text.length > 0) {
        NSString *keyWords = textField.text > 0 ? textField.text : _hotStr;
        ...
        [RequestManager requestWithMethod:GET WithUrlPath:encodedString params:nil from:nil showHud:false loadingText:nil enableUserActions:false success:^(id responseDic) {
            NSString *resultCode = [NSString stringWithFormat:@"%@",[responseDic objectForKey:@"result"]];
            if ([resultCode isEqualToString:@"ok"]) {
                if ([responseDic objectForKey:@"object"] && [[responseDic objectForKey:@"object"] isKindOfClass:[NSArray class]]) {
                    ...
                    if (objArr.count > 0) {
                        ...
                        NoResultBlock block = _noResultHanld;
                        if (block) {
                            block(keyWords);
                        }
                    }
...
-(void)rightClearHandle{
    [self endEditing:true];
    CloseBtnActionBlock block = _closeHanld;
    if (block) {
        block();
    }
}
...
-(void)dynmicSearchWithKeyWords:(NSString *)keyWords{
    if (keyWords.length >= 2) {
        ...
        [RequestManager requestWithMethod:POST WithUrlPath:str params:params from:nil showHud:false loadingText:nil enableUserActions:false success:^(id responseDic) {
            NSString *resultCode = [NSString stringWithFormat:@"%@",[responseDic objectForKey:@"result"]];
            if ([resultCode isEqualToString:@"ok"]) {
                _correlationArr = [NSMutableArray array];
                if ([responseDic objectForKey:@"object"] && [[responseDic objectForKey:@"object"] isKindOfClass:[NSDictionary class]]) {
                    ...
                    if ([objDic objectForKey:@"list"] && [[objDic objectForKey:@"list"] isKindOfClass:[NSArray class]] ) {
                        ...
                        if (_correlationArr.count > 0) {
                            ...
                            [self bringSubviewToFront:_correlationView];
                        }else{
